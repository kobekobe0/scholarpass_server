...
export const createResetPasswordAdmin = async (req, res) => {
    const {studentNumber} = req.body;
...
export const resetPasswordAdmin = async (req, res) => {
    ...
    try{
        ...
        const admin = await Admin.findOne({email: resetPassword.email});
        if(!admin){
        ...
        await createSystemLog("RESET_PASSWORD", "ADMIN", admin._id, "Admin", `Admin ${admin.email} reset password`, null);
